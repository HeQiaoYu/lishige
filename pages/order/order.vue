				...
				>
					...
					<view class="youhuiBox">
						<view class="yhItem myOdItem" v-for="(item,index) in tabItem.orderList" :key="index">
							<block v-for="(shop,i) in item.goods">
								...
								<view  v-for="(goods,ig) in shop.goods" :id="item.id"  class="indProItem uni-media-list odProItem"  @click="goOdInfo">
										...
										<view class="">
											...
											<view class="indProDbKuai flexBox between">
												<view class="indPro-money">
													<text>{{goods.specs?goods.specs:''}}</text>
												</view>
											</view>
										</view>
							...
							<view class="action-box b-t" v-if="item.status == '0'">
								...
								<button class="action-btn recom" @click="goPay(item.id)">{{item.nextstatus}}</button>
							</view>
...
<script>
	...
	import {orderList,cancelOrder} from '@/api/public'
	export default {
		components: {
			uniLoadMore,
			empty
		...
		methods: {
			...
			loadData(source){
				...
				orderList(state, page, openid).then(data => {
