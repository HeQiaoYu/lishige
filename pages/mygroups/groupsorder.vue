				...
				>
					...
					<view class="youhuiBox">
						<view class="yhItem myOdItem" v-for="(item,index) in tabItem.orderList" :key="index">
							...
							<view class="odItem-caozuo flexBox" v-if="item.status == '0'">
								<button class="odItBtn" @click="cancelOrder(item)">取消订单</button>
								 <block v-if="item.status==0">
									<navigator class="odItBtn signBtn-orangeBorder" :url="'/pages/money/grouppay?id='+item.id+'&teamid='+item.teamid">立即支付</navigator>
								</block>
							</view>
...
<script>
	...
	import {grouporderList,groupsordercancel} from '@/api/public'
	export default {
		components: {
			uniLoadMore,
			empty
		...
		methods: {
			...
			loadData(source){
				...
				grouporderList(state, page).then(data => {
					...
					navItem.loadingType = data.pagesize==data.list.length?'more':'noMore';
				})
			...
			cancelOrder(item){
				...
				groupsordercancel(item.id,openid).then(res => {
					let [error, on] = res, data = on.data;
			...
			goOdInfo(e) {
				...
				uni.navigateTo({
					url: `/pages/mygroups/orderDetails?id=`+id
				})
			},
			goPay() {
				uni.navigateTo({
