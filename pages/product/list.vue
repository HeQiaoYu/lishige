...
<script>
	...
	import { getCategory,getCategoryGoods } from "@/api/public";
	export default {
		components: {
			uniLoadMore	
		},
		...
		methods: {
			...
			async loadData(type='add', loading) {
				...
				getCategoryGoods(that.cateId,that.page).then(data => {
					let [error, on] = data,res = on.data;
					this.loadingType  = res.pagesize > 20 ? 'nomore' : 'more';
					//筛选，测试数据直接前端筛选了
